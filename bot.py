...
async def ask_contact_after_test(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await q.message.reply_text(
        ...
        "Если кнопка не видна — нажми на иконку 🎹 рядом с полем ввода.",
        reply_markup=ReplyKeyboardMarkup(
            [[KeyboardButton("📱 Поделиться контактом", request_contact=True)]],
            one_time_keyboard=True,
            resize_keyboard=True,
            input_field_placeholder="👆 Нажми кнопку выше",
        ),
    )
...
async def handle_test_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Логируем отправку контакта в Google Sheets
    ...
    # Уведомляем администратора (после результатов, чтобы ошибка не блокировала пользователя)
    try:
        await notify_admin(
            context,
            user=update.message.from_user,
